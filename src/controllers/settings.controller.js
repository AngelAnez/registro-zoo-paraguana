...
export const getSettings = (req, res) => {
  renderSettings(req, res, DEFAULT_ALERT);
};

export const renderSettings = async (req, res, alert) => {
  const { username, admin } = req.user;
  try {
    const [configsQuery] = await pool.query("SELECT * FROM configs LIMIT 1");
    const configDocument = configsQuery[0];
    const { internetDolar, defaultDolar, kidsPrice, adultsPrice } =
      configDocument;

    const { showAlert, messageAlert, typeAlert } = alert;

    res.render("app/modules/settings/settings", {
      username,
      admin,
      internetDolar,
      defaultDolar,
...
export const updateSettings = async (req, res) => {
  ...
  if (!internetDolarRegex.test(configData.internetDolar)) {
    return renderSettings(req, res, {
      showAlert: true,
      messageAlert:
        "Se ha producido un error al momento de guardar los cambios",
      typeAlert: "danger",
    });
  }
  ...
  renderSettings(req, res, {
    showAlert: true,
    messageAlert: "Los cambios han sido guardados exitosamente",
    typeAlert: "success",
  });
};
